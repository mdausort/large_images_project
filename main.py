...
python3 train.py -pw {pw} -ph {ph} -sp {sp} -m {m} -com {com} --num_epochs {num_epochs} --momentum {momentum} {cluster} {wandb_} --freezed_bb {freezed_bb} --model {model} --name_run {m}_{lr}_{bs}_{model}_{freezed_bb}
"""
...
if __name__=="__main__":
    ...
    parser.add_argument("--set_percent", type=float, nargs='+', default=[0.7, 0.1, 0.2], choices=['adult', 'child'], help="Patient identifier")
    ...
    parser.add_argument("--cluster", type=str, default='', choices=['', '--cluster'], help="Use of clusters")
    ...
    if args.task == 'csv':
        ...
        os.system("sbatch "+ str(os.join.path(PATH, sh_file_name)))
        
    elif args.task == 'train':
        sh_file_name = f"train_{args.bs}_{args.lr}_{args.num_epochs}_{args.momentum}_{args.model_name}_{freezed_backbone}.sh"
        with open(os.path.join(PATH, sh_file_name),"w") as writer:
            writer.write(train_file.format(pw=args.pw, ph=args.ph,
                                           sp=args.sp,
                                           m=args.m,
                                           p=args.p,
                                           set_percent=args.set_percent,
                                           ...
                                           wandb_=args.wandb_,
                                           freezed_backbone=args.freezed_bb, 
                                           freezed_backbone=freezed_backbone)) 
            writer.close()
